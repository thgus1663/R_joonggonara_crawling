...
testf=function(box){
  box111=box$findChildElement('tag name', 'a')
  tit1=box111$findChildElement('class','tit')
  return(as.character(tit1$getElementText()))
}

b=remDr$findElements('class', 'board_box ')
length(b)

box111=box11[[i]]$findChildElement('tag name', 'a')
tit1=box111$findChildElement('class','tit')
title=c(title,as.character(tit1$getElementText())) #제목을 벡터에 저장
url=c(url,as.character(box1$getElementAttribute('href')))
...
box2=url_save(skt_kt_lgu)



box11=lapply(skt_kt_lgu, url_save)
length(box11)
